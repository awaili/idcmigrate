...
def _mat(server, product, spec, region="ap-shanghai"):
...
def test_load_pricebook_fixture_default():
    ...
    # CVM SA5.MEDIUM4 ap-shanghai has a monthly price
    m, y = _lookup(book, "CVM", "SA5.MEDIUM4", "ap-shanghai")
    assert m > 0 and y > 0
...
def test_lookup_falls_back_to_product_default_then_zero():
    ...
    m, y = _lookup(book, "CVM", "SA5.NONEXISTENT", "ap-shanghai")
    assert m > 0  # by_product.CVM.ap-shanghai monthly
    # unknown product -> 0
    m2, y2 = _lookup(book, "MadeUpProduct", "x", "ap-shanghai")
...
def test_estimate_server_uses_book_and_basis():
    ...
    m = _mat(s, "CVM", "SA5.MEDIUM4", "ap-shanghai")
...
def test_estimate_portfolio_sums_and_rollups():
    ...
    matches = [
        _mat(s1, "CVM", "SA5.MEDIUM4", "ap-shanghai"),    # 650/yr
        _mat(s2, "CDB", "MySQL 8.0 MYSQL.HighMem - 8GB 高可用版(HA)", "ap-shanghai"),  # 1190/yr
        _mat(s3, "CVM", "SA5.SMALL2", "ap-beijing"),      # 280/yr
    ]
    out = estimate_portfolio([s1, s2, s3], matches, book)
    assert out["cloud_yearly"] == pytest.approx(650 + 1190 + 280, abs=0.5)
    assert out["per_product"]["CVM"] == pytest.approx(650 + 280, abs=0.5)
    assert out["per_product"]["CDB"] == pytest.approx(1190, abs=0.5)
    assert out["per_region"]["ap-shanghai"] == pytest.approx(650 + 1190, abs=0.5)
...
def test_estimate_portfolio_retain_retire_contribute_zero_cloud_cost():
    ...
    s1 = _srv("a1", app_ids=["app1"])
    s2 = _srv("a2", app_ids=["app2"])
    matches = [_mat(s1, "CVM", "SA5.MEDIUM4", "ap-shanghai"),
               _mat(s2, "CVM", "SA5.MEDIUM4", "ap-shanghai")]
...
# -- what-if ----------------------------------------------------------------
def test_what_if_region_returns_delta():
    reset_settings()
    book = load_pricebook(get_settings(), refresh=True)
    s = _srv("a1", app_ids=["app1"])
    matches = [_mat(s, "CVM", "SA5.MEDIUM4", "ap-shanghai")]  # 650/yr
    out = what_if([s], matches, book, region="ap-guangzhou")  # 630/yr
    assert out["applied"]["region"] == "ap-guangzhou"
    assert out["baseline"]["cloud_yearly"] == pytest.approx(650, abs=0.5)
    assert out["alternate"]["cloud_yearly"] == pytest.approx(630, abs=0.5)
    assert out["delta"] == pytest.approx(-20, abs=0.5)
...
def test_overrides_set_contract_source_and_onprem_rate(tmp_path, monkeypatch):
    ov = {"onprem_rate": 100,
          "CVM:SA5.MEDIUM4:ap-shanghai": {"monthly": 50, "yearly": 540}}
