/* ---------- copilot ---------- */
async function doAsk(){ const q=$('askQ').value.trim(); if(!q)return; $('askOut').innerHTML='<span class="spinner"></span>'; try{const r=await api('/ask',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({question:q})}); $('askOut').innerHTML=`<pre>${esc(r.answer)}</pre>`;}catch(e){ $('askOut').innerHTML='<span class="ev-err">Error: '+esc(String(e))+'</span>'; } }
